...
public class trace_spectralnorm
{
        ...
        public static void runIter(int n) {
                ...
                new trace_spectralnorm().Approximate(n);
        }
        
        private final double Approximate(int n) {
                System.out.println("private final double Approximate(int n) {");
                // create unit vector
        ...
        private final double A(int i, int j){
                System.out.println("private final double A(int i, int j){");
                return 1.0/((i+j)*(i+j+1)/2 +i+1);
        }
        
        /* multiply vector v by matrix A */
        private final void MultiplyAv(int n, double[] v, double[] Av){
                System.out.println("private final void MultiplyAv(int n, double[] v, double[] Av){");
                for (int i=0; i<n; i++){
                        System.out.println("for (int i=0; i<n; i++){");
                        Av[i] = 0;
                        for (int j=0; j<n; j++) {
        ...
        private final void MultiplyAtv(int n, double[] v, double[] Atv){
                System.out.println("private final void MultiplyAtv(int n, double[] v, double[] Atv){");
                for (int i=0;i<n;i++){
                        System.out.println("for (int i=0;i<n;i++){");
                        Atv[i] = 0;
                        for (int j=0; j<n; j++) {
        ...
        private final void MultiplyAtAv(int n, double[] v, double[] AtAv){
                System.out.println("private final void MultiplyAtAv(int n, double[] v, double[] AtAv){");
                double[] u = new double[n];
                MultiplyAv(n,v,u);
