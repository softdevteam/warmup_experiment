...
def eval_A (i, j):
    print "def eval_A (i, j):"
    return 1.0 / (((i + j) * (i + j + 1) >> 1) + i + 1)

def eval_A_times_u (u, resulted_list):
    print "def eval_A_times_u (u, resulted_list):"
    u_len = len (u)
    local_eval_A = eval_A
    
    for i in xrange (u_len):
        print "for i in xrange (u_len):"
        partial_sum = 0
...
def eval_At_times_u (u, resulted_list):
    print "def eval_At_times_u (u, resulted_list):"
    u_len = len (u)
    local_eval_A = eval_A
...
def eval_AtA_times_u (u, out, tmp):
    print "def eval_AtA_times_u (u, out, tmp):"
