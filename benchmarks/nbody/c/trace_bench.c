...
void n_advance(int nbodies, struct planet * bodies, double dt) {
  printf("void n_advance(int nbodies, struct planet * bodies, double dt) {\n");

  int i, j;

  for (i = 0; i < nbodies; i++) {
    printf("for (i = 0; i < nbodies; i++) {\n");
    struct planet * b = &(bodies[i]);
    for (j = i + 1; j < nbodies; j++) {
      printf("for (j = i + 1; j < nbodies; j++) {\n");
...
double energy(int nbodies, struct planet * bodies) {
  printf("double energy(int nbodies, struct planet * bodies) {\n");
  double e;
  int i, j;
...
void offset_momentum(int nbodies, struct planet * bodies) {
  printf("void offset_momentum(int nbodies, struct planet * bodies) {\n");
...
void inner_iter(int n) {
  ...
  if (abs(checksum - EXPECT_CHECKSUM) >= EPSILON) {
    printf("if (abs(checksum - EXPECT_CHECKSUM) >= EPSILON) {\n");
...
void run_iter(int n) {
  printf("void run_iter(int n) {\n");
   ...
   if ((bodies = malloc(sizeof(initial_bodies))) == NULL) {
     printf("if ((bodies = malloc(sizeof(initial_bodies))) == NULL) {\n");
