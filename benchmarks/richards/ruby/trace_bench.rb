...
def main(reps)
   ...
   for i in 0..reps 
     ...
     if s.getCounts != [9297, 23246]
        puts "if s.getCounts != [9297, 23246]"
        return false
     end
   end
...
class Scheduler
   ...
   def queue(packet)
      puts "def queue(packet)"
      task = @table.at(packet.id)
      if task # not nil
         puts "if task # not nil"
         @queueCount += 1
         packet.link = nil
         packet.id = @currentId
   ...
   def release(id)
      ...
      if task.pri > @currentTcb.pri 
         puts "if task.pri > @currentTcb.pri "
         task
      else
         puts "else"
         @currentTcb
   ...
   def schedule()
      ...
      while @currentTcb # not nil        
         puts "while @currentTcb # not nil        "
         if @currentTcb.isHeldOrSuspended?     
            puts "if @currentTcb.isHeldOrSuspended?     "
   ...
   def run(packet)   
      ...
      if packet # not nil    
         ...
         if packet.kind == Packet::WORK
            puts "if packet.kind == Packet::WORK"
            @v1 = packet.addTo(@v1)             
         else
            puts "else"
            @v2 = packet.addTo(@v2)                    
...
class IdleTask   
   def initialize(scheduler,v1,v2)
      puts "def initialize(scheduler,v1,v2)"
      @scheduler = scheduler
      @v1 = v1
      @v2 = v2   
   end   
...
class Tcb
   ...
   def initialize(link, id, pri, wkq, task)
      puts "def initialize(link, id, pri, wkq, task)"
      @link = link
      @id = id
      @pri = pri
      @wkq = wkq     
   ...
   def checkPriorityAdd(task,packet)
      puts "def checkPriorityAdd(task,packet)"
      if @wkq # not nil
         puts "if @wkq # not nil"
         packet.addTo(@wkq)      
      else      
         ...
         @wkq = packet
         @state |= 0b1 # RUNNABLE      
         if @pri > task.pri then
          puts "if @pri > task.pri then"
          return self 
         end
      end      
      task
   ...
   def run 
      ...
      if @state == 0b11 # SUSPENDED_RUNNABLE   
         puts "if @state == 0b11 # SUSPENDED_RUNNABLE   "
         packet = @wkq
         @wkq = packet.link         
...
class Packet
   ...
   def initialize(link, id, kind)
      puts "def initialize(link, id, kind)"
      @link = link
      @id = id
      @kind = kind
      @a1 = 0
   ...
   def addTo(queue)
     ...
     unless queue
       ...
       while (peek = nextPacket.link) 
         puts "while (peek = nextPacket.link) "
         nextPacket = peek
       end
